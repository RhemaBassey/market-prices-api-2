...
def cryptoPriceUSD(cryptoName,amount):
  url = "https://www.coingecko.com/en/coins/"+cryptoName.lower()
  cryptos = url_link(url).find_all("div")
...
def stockPriceUSD(tickerSymbol,amount):
  ...
  try:
    stockPrice = url_link(url).find("fin-streamer", attrs={'data-symbol':tickerSymbol}).text
...
def printAssetData(toggle,asset_name, amount,price, value):
  if (amount or price) == '':
    print(str(asset_name) + " => $ " + str(value))
      ...
      print(( str(asset_name) +", "+ str(amount)+", " + str(price) + " => $ " + str(value)))
...
output_data = ["\n", [display_date_time()], ["ASSET CLASS","AMOUNT","PRICE","VALUE"]]
...
for asset_tag in asset_tags[start:stop]:
  ...
  loadMessage(asset_data_tag.replace("_", " "), "(" + str(index - start +1)+"/"+str(stop - start)+")")
...
#    and uploads the result information (cryptos_data,stocks_data) into their respective output folder-----------
  ...
  if(index == len(asset_tags)-1):
    total_assets_data = [output_data[0], output_data[1], [output_data[2][0],output_data[2][-1]]]
    total_holdings = 0
    print("----------------------------------------------------------------------------")
    for asset_tag in asset_tags[start:stop]:
      asset_holdings_tag = asset_tag[2]
      ...
      asset_holdings_tag = asset_holdings_tag[0].upper() + asset_holdings_tag[1:].replace("_h", " H")
      total_assets_data.append([ asset_holdings_tag + " Total", asset_holdings])
      total_holdings += asset_holdings

      print(asset_holdings_tag + " ----------> $ " + str(round(asset_holdings,2)))
    total_assets_data.append(['TOTAL:','','', round((total_holdings),2)])
    file_action('total_assets_data', 'a',total_assets_data)
    print("----------------------------------------------------------------------------")
    print("TOTAL ASSETS  ==========> ", '$ ' + str(round(total_holdings,2)))
